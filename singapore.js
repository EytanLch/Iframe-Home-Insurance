/*
  Singapore Choropleth with Leaflet: https://choropleth.sylvaindurand.org
  Copyright 2015 Sylvain Durand – released under the MIT license.
*/

function singapore( map ) {


    this.init = function() {

                  // Download geospatial vector data and metadata
                  d3.json( 'data/geo/PLAN_BDY_DWELLING_TYPE_2016.json', function ( e, json ) {
                    d3.csv( 'data/geo/test_data3.csv', function ( e, data ) {

                      // Convert TopoJSON to GeoJSON
                      $.topo = topojson.feature( json, json.objects["PLAN_BDY_DWELLING_TYPE_2016"] );
                    ...
                    new L.GeoJSON($.topo, {
                      ...
                      style: function( feature ) {
                               return {
                                 ...
                                 fillOpacity:
                                  // $.data[ feature.id ] ? ($.data[ feature.id ][$.stat] ? Math.max(alpha( $.data[ feature.id ][$.stat] ), 1 ) : 0) : .2,
                                   $.data[ feature.id ] ? ($.data[ feature.id ][$.stat] ? 1 : 0) : .2,
  ...
  this.legend = function() {
                  ...
                  var
                        ...
                        input = div.append( "input" )
                                     .attr( "class", "value" )
                                     .attr( "disabled", "" )
                                     .attr( "placeholder", "Move mouse on territory" ),
                  ...
                  search.on( 'awesomplete-selectcomplete', function() {
                    ...
                    for ( var i in $.data ) {
                      if ( value == $.data[i].name + " ( " + $.data[i].postcode + " )" ) {
                        ...
                        map.flyTo( L.latLng( $.data[i].y, $.data[i].x ), 15);
